...
use_derivative, k_d = os.path.exists('/data/use_deriv'), 0
if use_derivative:
  with open('/data/use_deriv', 'r') as f:
    k_d = float(f.read().strip())
...
class LongPIDController:
  def __init__(self, k_p, k_i, k_d, k_f=1., pos_limit=None, neg_limit=None, rate=100, sat_limit=0.8, convert=None):
    self.enable_long_derivative = os.path.exists('/data/long_deriv')
    self._k_p = k_p  # proportional gain
    self._k_i = k_i  # integral gain
